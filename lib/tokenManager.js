  ...
  try {
    ...
    if (!forceRefresh && currentToken) {
      try {
        ...
        console.log(
          "TokenManager: Token validation failed, refreshing...",
          validationError
        );
    ...
    console.log("TokenManager: Refreshing Firebase token...");
...
export const clearToken = () => {
  ...
  console.log("TokenManager: Token and auth state cleared");
};
...
export const forceRefresh = async () => {
  try {
    ...
    console.log("TokenManager: No user available for force refresh");
    return null;
  } catch (error) {
    console.error("TokenManager: Error during force refresh:", error);
    return null;
  }
};
...
export const validateTokenWithFirebase = async () => {
  try {
    ...
    if (!currentUser || !currentToken) {
      console.log("TokenManager: No user or token available for validation");
      return false;
    }
    ...
    console.log(`TokenManager: Token validation result: ${isValid}`);
    return isValid;
  } catch (error) {
    console.error("TokenManager: Error validating token with Firebase:", error);
    return false;
  }
};
...
export const ensureValidTokenBeforeApiCall = async () => {
  try {
    ...
    console.log("TokenManager: Valid token ensured for API call");
    return token;
  } catch (error) {
    console.error("TokenManager: Failed to ensure valid token:", error);
    throw error;
  }
};
...
export const initializeTokenManager = async () => {
  try {
    console.log("TokenManager: Initializing global token management...");
    await initializeAuthState();
    console.log("TokenManager: Global token management initialized");
    return true;
  } catch (error) {
