...
const Dialog = ({ children, open: openProp, onOpenChange }) => {
  ...
  return (
    <DialogContext.Provider value={{ open: isOpen, setOpen: handleOpenChange }}>{children}</DialogContext.Provider>
  );
};
...
const DialogClose = React.forwardRef(({ className, children, asChild = false, ...props }, ref) => {
  const { setOpen } = useDialog();
